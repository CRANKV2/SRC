...
ui_print "*   ⚡️Modified by CRANKV2 @ GitHub⚡️  *"
ui_print "**************************************"
ui_print " "
...
if ! $BOOTMODE; then
  ui_print "[⚡️] Only uninstall is supported in recovery"
  ui_print "[⚡️] Uninstalling!"
...
# Debug
if $DEBUG; then
  ui_print "[⚡️] Debug mode"
  ui_print "[⚡️] Module install log will include debug info."
  ui_print ""
  set -x
fi

# Extract files
ui_print "[⚡️] Extracting Simple Res. Change Module files..."
unzip -o "$ZIPFILE" -x 'META-INF/*' 'common/functions.sh' -d $MODPATH >&2
[ -f "$MODPATH/common/addon.tar.xz" ] && tar -xf $MODPATH/common/addon.tar.xz -C $MODPATH/common 2>/dev/null

# Run addons
if [ "$(ls -A $MODPATH/common/addon/*/install.sh 2>/dev/null)" ]; then
  ui_print " "; ui_print "[⚡️] Running Addons..."
  for i in $MODPATH/common/addon/*/install.sh; do
    ui_print "[⚡️] Running $(echo $i | sed -r "s|$MODPATH/common/addon/(.*)/install.sh|\1|")..."
    . $i
  done
fi
...
### Install
ui_print "[⚡️] Installing Simple Res. Change Module..."
